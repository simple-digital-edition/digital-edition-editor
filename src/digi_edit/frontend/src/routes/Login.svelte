...
<div class="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 border border-solid border-gray-300 shadow-lg w-96">
    <h1 class="px-4 py-2 bg-gray-300 font-bold text-lg">Log in</h1>
    <form on:submit={login} class="px-4 py-2">
        <label class="block mb-3 text-sm">E-Mail Address
            <input bind:value={email} type="email" class="block px-1 py-1 border border-solid {emailError ? 'border-red-600' : 'border-gray-300'} w-full text-base focus:shadow-inner" />
            {#if emailError}
                <span class="block px-2 py-1 text-sm text-white bg-red-600">{emailError}</span>
            {/if}
        </label>
        <label class="block mb-3 text-sm">Password
            <input bind:value={password} type="password" class="block px-1 py-1 border border-solid {passwordError ? 'border-red-600' : 'border-gray-300'} w-full text-base focus:shadow-inner" />
            {#if passwordError}
        ...
        <div class="text-right">
            {#if loggingIn}
                <button class="inline-block px-3 py-1 bg-blue-700 text-white hover:bg-blue-600 focus:bg-blue-600 transition-colors">Logging in</button>
            {:else}
                <button class="inline-block px-3 py-1 bg-blue-700 text-white hover:bg-blue-600 focus:bg-blue-600 transition-colors">Log in</button>
            {/if}
        </div>
    </form>
</div>
